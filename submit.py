...
Original file is located at
    https://colab.research.google.com/drive/1CFU9q9gbSC-E12E9l7gp40wVXh9uSv3i
"""
...
class Node:
	...
	# Function get_entropy takes in the parameter ’counts’ and calculates the entropy of the corpus ’counts’
	def get_entropy(self,counts):
		...
		if num_elements <=1:
			# print(f"warning")
			return 0
		# Calculate the proportion of each element in ’counts’
		proportions = counts / num_elements
		# Calculate the entropy of the distribution
		return np.sum( proportions * np.log2( counts ) )
	
	def partition(self,all_words,ind,query):
		split_dict={}
		count_dict={}

		for i in ind:
			if(len(all_words[int(i)])!=len(query)):
				continue
			mask = self.reveal(all_words[int(i)], query)

		if mask not in split_dict:
			split_dict[ mask ] = []
			count_dict[ mask ] = 0
		split_dict[ mask ].append( all_words[int(i)] )
		count_dict[ mask ] += 1
		entropy = self.get_entropy(np.array(list(count_dict.values())))
		return ( entropy, split_dict )
	
	# Function best_word_index that takes in the parameters words and index and outputs the best guessed word’s index
	def best_word_index(self,words,ind):
		# Initialize the best_entropy variable to infinity
		best_entropy=np.inf
		# Initialize an empty dictionary called best_split_dict
		best_split_dict={}
		# Initialize the best_guess_ind variable to zero
		best_guess_ind=0
		# Initialize an empty string called best_guess
		best_guess=''
		# Iterate through each index i in the ind list
		for i in ind:
				# Call the partition function to split the words into two groups based on the word at index i
				entropy,split_dict=self.partition(words,ind,words[i])
				# If the entropy of the split is lower than the best entropy seen so far
				if entropy < best_entropy:
					# Update the best_entropy, best_split_dict, best_guess, and best_guess_ind variables
					best_entropy=entropy
					best_split_dict=split_dict
					best_guess=words[i]
					best_guess_ind=i
		# Return the index of the best guess word
		return best_guess_ind
	...
	def process_node( self, all_words, my_words_idx, history, verbose ):
		...
		if len( history ) == 0:
			...
			query_idx=self.best_word_index(all_words,my_words_idx)
